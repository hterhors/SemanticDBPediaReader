package de.hterhors.semanticmr.projects.single;
...
import de.hterhors.semanticmr.projects.single.specs.SingleSpecifications;
 ...
 *
 */
public class SingleSlotFillingExtraction extends AbstractSemReadProject {
	...
	public static void main(String[] args) {
		new SingleSlotFillingExtraction();
	}

	private static Logger log = LogManager.getFormatterLogger(SingleSlotFillingExtraction.class);
	...
	private final File slotPairConstraitsSpecifications = new File(
			"src/main/resources/dam/constraints/slotPairExclusionConstraints.csv");

	/**
	 * The directory of the corpus instances. In this example each instance is
	 * stored in its own json-file.
	 */
	private final File instanceDirectory = new File("src/main/resources/single/corpus/instances/");
	 ...
	 */
	private final File externalNerlaAnnotations = new File("src/main/resources/single/corpus/nerla/");

	public SingleSlotFillingExtraction() {
		...
		super(SystemScope.Builder.getScopeHandler()
				 ...
				 */
				.addScopeSpecification(SingleSpecifications.systemsScopeReader)
				/**
				 ...
				 */
				.build());
		System.out.println(EntityType.get("Single").getHierarchicalEntityTypes().size());
		System.out.println(EntityType.get("Person").getHierarchicalEntityTypes().size());
		System.out.println(EntityType.get("Band").getHierarchicalEntityTypes().size());
		System.out.println(EntityType.get("MusicalArtist").getHierarchicalEntityTypes().size());
		System.out.println(EntityType.get("Album").getHierarchicalEntityTypes().size());
		System.out.println(EntityType.get("Agent").getHierarchicalEntityTypes().size());
		System.out.println(EntityType.get("Genre").getHierarchicalEntityTypes().size());
...
//				slotPairConstraitsSpecifications);
		 ...
		 * provide some templates that implement standard features like context-,
		 * surface form, and linguistic dependency features etc. But also two data set
		 * specific templates for covering birth and death dates.
		 * 
		 */
		...
		IStateInitializer stateInitializer = ((instance) -> new State(instance,
				new Annotations(new EntityTemplate(AnnotationBuilder.toAnnotation("Single")))));
...
//		AbstractSampler sampler = new EpochSwitchSampler(e -> new Random(e).nextBoolean());
		 ...
		 */
		final File modelBaseDir = new File("models/single/");
		final String modelName = "single" + new Random().nextInt(1000000);
